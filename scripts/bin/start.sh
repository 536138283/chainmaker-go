...
VM_JAVA_IMAGE_NAME="chainmakerofficial/chainmaker-vm-engine-java:v2.4.0"
...
function start_vm_go() {
  ...
  preload_disable=$chainmaker_vm_common_contract_engine_preload_disable
  preload_num_by_use_frequency=$chainmaker_vm_common_contract_engine_preload_num_by_use_frequency
  preload_num_by_last_time=$chainmaker_vm_common_contract_engine_preload_num_by_last_time
...
if [ -z "${pid}" ];then
    ...
    # start chainmaker
    # use >> to append to prevent log loss due to restart
