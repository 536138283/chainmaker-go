...
CONFIG_TPL_PATH=${PROJECT_PATH}/config/config_tpl_ibc
...
CRYPTOGEN_TOOL_CONF=${CRYPTOGEN_TOOL_PATH}/config/ibc_crypto_config_template.yml
...
#    cp $CRYPTOGEN_TOOL_PKCS11_KEYS pkcs11_keys.yml
    ...
    ${CRYPTOGEN_TOOL_BIN} generate-ibc -c ./crypto_config.yml  #-p ./pkcs11_keys.yml
}
...
" node$i/chainmaker.yml
        ...
        for ((j = 1; j < $CHAIN_CNT + 1; j = j + 1)); do
            ...
            if  [ $NODE_CNT -eq 7 ] || [ $NODE_CNT -eq 13 ] || [ $NODE_CNT -eq 16 ]; then
                ...
                xsed "s%#\(.*\)master_keys:%\1master_keys:%g" node$i/chainconfig/bc$j.yml
            ...
            do
                ...
                cp $BUILD_CRYPTO_CONFIG_PATH/$file/ca/master_public.key $BUILD_CONFIG_PATH/node$i/certs/ca/$file
