...
if  [[ ! -n $node_count ]] ;then
    ...
    echo "  you can use "
    echo "             ./prepare_pk.sh nodeCount chainCount alreadyBuild enableDockerVm"
    echo "             ./prepare_pk.sh 4 1 false true"
    exit 1
fi
...
function start_chainmaker() {
  ...
  if [ "${alreadyBuild}" != "true" ]; then
    ...
    if [ "${enableDocker}" == "true" ]; then
      echo "【sh】 ./prepare_pk.sh 4 1 11391 12391 32391 22391 23391 -c 1 -l INFO --hash SHA256 -v true -j false --jlog=INFO  --vtp=tcp --vlog=INFO"
      ./prepare_pk.sh 4 1 11391 12391 32391 22391 23391 -c 1 -l INFO --hash SHA256 -v true -j false --jlog=INFO  --vtp=tcp --vlog=INFO
    else
      echo "【sh】 ./prepare_pk.sh 4 1 11391 12391 32391 22391 23391 -c 1 -l INFO --hash SHA256 -v false -j false --jlog=INFO  --vtp=tcp --vlog=INFO"
      ./prepare_pk.sh 4 1 11391 12391 32391 22391 23391 -c 1 -l INFO --hash SHA256 -v false -j false --jlog=INFO  --vtp=tcp --vlog=INFO
    fi
