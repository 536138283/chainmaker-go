...
func printConfig(wasmType string) {
	fmt.Printf("=========init %s=========\n", wasmType)
	fmt.Println("  docker contract path         : ", DockerGoContractPath)
	fmt.Println("  docker contract upgrade path : ", DockerGoContractUpgradePath)
	fmt.Println("  docker contract name: ", contractName)
	fmt.Println("  docker runtime type : ", runtimeType)
	fmt.Println()
}
...
func testCreate(sk3 crypto.PrivateKey, client *apiPb.RpcNodeClient, chainId string) {
	common.CreateContract(sk3, client, chainId, contractName, DockerGoContractPath, runtimeType)
}
...
func testUpgrade(sk3 crypto.PrivateKey, client *apiPb.RpcNodeClient, chainId string) {
	...
	resp := common.UpgradeContract(sk3, client, chainId, contractName, DockerGoContractUpgradePath, runtimeType)
