...
var (
	DockerGoContractPath        = ""
	DockerGoContractUpgradePath = ""
	contractName                = ""
...
func initDockerGoTest() {
	DockerGoContractPath = "./docker-go/docker-go-contract-7functions.7z"
	DockerGoContractUpgradePath = "./docker-go/contract20_upgrade.7z"
	contractName = "docker-go-contract-7functions"
	runtimeType = commonPb.RuntimeType_DOCKER_GO
}

func printConfig(wasmType string) {
	fmt.Printf("=========init %s=========\n", wasmType)
	fmt.Println("  docker go path         : ", DockerGoContractPath)
	fmt.Println("  docker go upgrade path : ", DockerGoContractUpgradePath)
	fmt.Println("  docker go contract name: ", contractName)
	fmt.Println("  docker go runtime type : ", runtimeType)
	fmt.Println()
}
...
func testCreate(sk3 crypto.PrivateKey, client *apiPb.RpcNodeClient, chainId string) {
	...
	wasmBin, _ := ioutil.ReadFile(DockerGoContractPath)
	var pairs []*commonPb.KeyValuePair
...
func testUpgrade(sk3 crypto.PrivateKey, client *apiPb.RpcNodeClient, chainId string) {
	...
	wasmBin, _ := ioutil.ReadFile(DockerGoContractUpgradePath)
	var pairs []*commonPb.KeyValuePair
