...
def UpdateSetting():
    gl.TESTPROJECTPATH= r"/Users/devinzeng/go/src/chainmaker.org/chainmaker-go/test/chain1/bin"
    gl.CMC_TOOL_PATH = "cd /Users/devinzeng/go/src/chainmaker.org/chainmaker-go/test/chain1/bin && "
    ...
    gl.ADMIN_CRT_FILE_PATHS = ','.join([f'{gl.CRYPTO_CONFIG_PATH}/wx-org{i}.chainmaker.org/certs/user/admin1/admin1.sign.crt'
                                 for i in range(1, 4)])
    gl.ADMIN_ORG_IDS_PWK = ','.join([f'wx-org{i}.chainmaker.org' for i in range(1, 4)])
    gl.ADMIN_KEY_FILE_PATHS_BY_PWK = ','.join([f'{gl.CRYPTO_CONFIG_PATH}/wx-org{i}.chainmaker.org/admin/admin.key'
                                        for i in range(1, 4)])
