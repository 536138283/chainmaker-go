...
func TestCommitter_Commit_getRetryListAfterABA(t *testing.T) {
	...
	allTrans[txList[2].Header.TxId] = txList[2]
	allTrans[txList[3].Header.TxId] = txList[3]
	allTrans[txList[4].Header.TxId] = txList[4]
	...
	for _, v := range c.retryList {
		fmt.Println("retryList:", v.Header.TxId)
	}
...
func createBatch(blockHash []byte, height int64, txList []*commonpb.Transaction) *commonpb.Block {
	b0 := cache.CreateNewTestBlock(height)
	hash0 := blockHash
