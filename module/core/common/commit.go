...
func (chain *BlockCommitter) isBlockLegal(blk *commonpb.Block) error {
...
func (chain *BlockCommitter) AddBlock(block *commonpb.Block) error {
...
func (chain *BlockCommitter) monitorCommit(bi *commonpb.BlockInfo) error {
	if !localconf.ChainMakerConfig.MonitorConfig.Enabled {
		return nil
	}
	raw, err := proto.Marshal(bi)
...
func (chain *BlockCommitter) syncWithTxPool(block *commonpb.Block, height int64) []*commonpb.Transaction {
...
func (chain *BlockCommitter) notifyChainConf(block *commonpb.Block, err error) error {
	if block != nil && block.GetTxs() != nil && len(block.GetTxs()) > 0 {
		tx := block.GetTxs()[0]
		if _, ok := chainconf.IsNativeTx(tx); ok {
...
func (chain *BlockCommitter) rearrangeRWSet(block *commonpb.Block, rwSetMap map[string]*commonpb.TxRWSet) []*commonpb.TxRWSet {
...
func (chain *BlockCommitter) checkLastProposedBlock(block *commonpb.Block, lastProposed *commonpb.Block,
