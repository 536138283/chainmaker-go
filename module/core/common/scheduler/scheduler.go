				...
				err := goRoutinePool.Submit(func() {
					...
					if !applyResult {
						...
						ts.log.Debugf("apply to snapshot tx id:%s, result:%+v, apply count:%d",
							tx.Header.GetTxId(), txSimContext.GetTxResult(), applySize)
					}
...
func (ts *TxScheduler) simulateTx(tx *commonpb.Transaction, snapshot protocol.Snapshot, block *commonpb.Block) (
	...
	if txResult, specialTxType, err = ts.runVM(tx, txSimContext); err != nil {
		...
		ts.log.Errorf("failed to run vm for tx id:%s, tx result:%+v, error:%+v",
			tx.Header.GetTxId(), txResult, err)
	}
	...
	*commonpb.Result, protocol.ExecOrderTxType, error) {
	...
	case commonpb.TxType_QUERY_SYSTEM_CONTRACT, commonpb.TxType_QUERY_USER_CONTRACT:
		...
		if err := proto.Unmarshal(tx.RequestPayload, &payload); err != nil {
			return errResult(result, fmt.Errorf("failed to unmarshal query payload for tx %s, %s",
				tx.Header.TxId, err))
		}
	...
	case commonpb.TxType_INVOKE_USER_CONTRACT:
		...
		if err := proto.Unmarshal(tx.RequestPayload, &payload); err != nil {
			return errResult(result, fmt.Errorf("failed to unmarshal transact payload for tx %s, %s",
				tx.Header.TxId, err))
		}
		contractName = payload.ContractName
		method = payload.Method
	...
	case commonpb.TxType_INVOKE_SYSTEM_CONTRACT:
		...
		var err error
		if err = proto.Unmarshal(tx.RequestPayload, &payload); err != nil {
			return errResult(result, fmt.Errorf("failed to unmarshal invoke payload for tx %s, %s",
				tx.Header.TxId, err))
		}
		contractName = payload.ContractName
		method = payload.Method
		...
		if ac, err = txSimContext.GetAccessControl(); err != nil {
			ts.log.Errorf("failed to get access control from tx sim context for tx: %s, error: %s, contract[%s], "+
				"method[%s]", tx.Header.TxId, err.Error(), contractName, method)
			return errResult(result, fmt.Errorf("failed to get access control from tx sim context for tx: %s, "+
				"error: %s", tx.Header.TxId, err.Error()))
		}

		if !ac.ResourcePolicyExists(method) {
			ts.log.Infof("policy not found for invoke_system_contract, contract[%s], method[%s], skip acVerify here",
				contractName, method)
		} else {
			...
			if endorsements == nil {
				return errResult(result, fmt.Errorf("endorsements not found in invoke system contract payload, "+
					"tx id:%s", tx.Header.TxId))
			}
	...
	case commonpb.TxType_UPDATE_CHAIN_CONFIG:
		var payload commonpb.SystemContractPayload
		if err := proto.Unmarshal(tx.RequestPayload, &payload); err != nil {
			return errResult(result, fmt.Errorf("failed to unmarshal system contract payload for tx %s, %s",
				tx.Header.TxId, err.Error()))
		}
		contractName = payload.ContractName
		method = payload.Method
		parameterPairs = payload.Parameters
		...
		if endorsements == nil {
			return errResult(result, fmt.Errorf("endorsements not found in config update payload, tx id:%s",
				tx.Header.TxId))
		}
		payload.Endorsement = nil
		verifyPayloadBytes, err := proto.Marshal(&payload)
	...
	case commonpb.TxType_MANAGE_USER_CONTRACT:
		...
		if err := proto.Unmarshal(tx.RequestPayload, &payload); err != nil {
			return errResult(result, fmt.Errorf("failed to unmarshal contract mgmt payload for tx %s, %s",
				tx.Header.TxId, err.Error()))
		}
		...
		if endorsements == nil {
			return errResult(result, fmt.Errorf("endorsements not found in contract mgmt payload, tx id:%s",
				tx.Header.TxId))
		}
	...
	if len(parameters) > protocol.ParametersKeyMaxCount {
		return errResult(result, fmt.Errorf("expect less than %d parameters, but got %d, tx id:%s",
			protocol.ParametersKeyMaxCount, len(parameters), tx.Header.TxId))
	}
	for key, val := range parameters {
		if len(key) > protocol.DefaultStateLen {
			return errResult(result, fmt.Errorf("expect key length less than %d, but got %d, tx id:%s",
				protocol.DefaultStateLen, len(key), tx.Header.TxId))
		}
		match, err := regexp.MatchString(protocol.DefaultStateRegex, key)
		if err != nil || !match {
			return errResult(result, fmt.Errorf("expect key no special characters, but got key:[%s]. letter, number, "+
				"dot and underline are allowed, tx id:[%s]", key, tx.Header.TxId))
		}
		if len(val) > protocol.ParametersValueMaxLength {
			return errResult(result, fmt.Errorf("expect value length less than %d, but got %d, tx id:%s",
				protocol.ParametersValueMaxLength, len(val), tx.Header.TxId))
		}
	}
	contractResultPayload, specialTxType, txStatusCode := ts.VmManager.RunContract(contractId, method, byteCode,
		parameters, txSimContext, 0, tx.Header.TxType)
...
func (ts *TxScheduler) acVerify(txSimContext protocol.TxSimContext, methodName string,
	endorsements []*commonpb.EndorsementEntry, msg []byte, parameters map[string]string) error {
	var ac protocol.AccessControlProvider
	...
	if ac, err = txSimContext.GetAccessControl(); err != nil {
		return fmt.Errorf("failed to get access control from tx sim context for tx: %s, error: %s",
			tx.Header.TxId, err.Error())
	}
	...
	for _, endorsement := range endorsements {
		if endorsement == nil || endorsement.Signer == nil {
			return fmt.Errorf("failed to get endorsement signer for tx: %s, endorsement: %+v",
				tx.Header.TxId, endorsement)
		}
		if endorsement.Signer.IsFullCert {
			...
			if fullMemberInfo, err := txSimContext.Get(commonpb.ContractName_SYSTEM_CONTRACT_CERT_MANAGE.String(),
				[]byte(memberInfoHex)); err != nil {
				return fmt.Errorf("failed to get full cert from tx sim context for tx: %s, error: %s",
					tx.Header.TxId, err.Error())
			} else {
	...
	if verifyResult, err := utils.VerifyConfigUpdateTx(methodName, fullCertEndorsements, msg,
		targetOrgId, ac); err != nil {
