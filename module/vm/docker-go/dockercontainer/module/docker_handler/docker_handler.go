...
import (
	...
	"chainmaker.org/chainmaker-go/docker-go/dockercontainer/module/udsserver"
...
// DockerHandler used to handle each sandbox's message
// to deal with each contract message
type DockerHandler struct {
	user      *security2.User
	logger    *log.Logger
	UdsServer *udsserver.UDSServer
...
func NewDockerHandler(user *security2.User, tx *outside.TxRequest, scheduler protocol.Scheduler) (*DockerHandler, error) {

	handler := &DockerHandler{
		logger:    utils.NewLogger("Docker Handler - " + tx.ContractName),
		tx:        tx,
		user:      user,
		state:     created,
		scheduler: scheduler,
	...
	udsServer, err := udsserver.NewUDSRpcServer(user, handler)
	if err != nil {
		return nil, err
	}

	handler.UdsServer = udsServer
	handler.logger.Println("Handler created")
	return handler, nil
}

func (h *DockerHandler) SetStream(stream protogo.Contract_ConnectServer) {
	h.stream = stream
}

func (h *DockerHandler) sendMessage(msg *protogo.ContractMessage) error {
...
func (h *DockerHandler) HandleMessage(msg *protogo.ContractMessage) error {
...
func (h *DockerHandler) handleCreated(registerMsg *protogo.ContractMessage) error {
...
func (h *DockerHandler) afterRegistered() error {
	if h.state != prepare {
...
func (h *DockerHandler) handlePrepare(readyMsg *protogo.ContractMessage) error {
	h.state = ready

	return h.afterFirstReady()
}

func (h *DockerHandler) afterFirstReady() error {
...
func (h *DockerHandler) sendInit() error {
...
func (h *DockerHandler) sendInvoke() error {
...
func (h *DockerHandler) handleReady(readyMsg *protogo.ContractMessage) error {
	//if h.state != prepare {
...
func (h *DockerHandler) handleGetState(getStateMsg *protogo.ContractMessage) error {
...
func (h *DockerHandler) handleCompleted(completedMsg *protogo.ContractMessage) error {
...
func (h *DockerHandler) afterCompleted() error {

	// close the current server
	h.UdsServer.FinishCh <- true
	h.UdsServer.Stop()
	return nil
}
