...
var log = logging.Logger("libp2ppubsub")
...
// WithUid set the unique id for pubsub
// WithUid 设置pubsub唯一标识
...
// WithWhitelist provides an implementation of the blacklist; the default is a MapWhitelist
// WithWhitelist 提供一个 Whitelist 实现，默认是 MapWhitelist。用于控制pubsub网络节点广播数据隔离
...
// WithMessageIdFn is an option to customize the way a message ID is computed for a libp2ppubsub message.
...
// withInternalTracer adds an internal event tracer to the libp2ppubsub system
...
// WithMaxMessageSize sets the global maximum message size for libp2ppubsub wire
...
// However, be wary of increasing the limit, as libp2ppubsub networks are naturally
...
// than being pushed proactively. Under this design, you'd use the libp2ppubsub layer
			...
			go func(c chan *RPC) {
				defer func() {
					if err := recover(); err != nil {
						log.Error("pubsub process loop defer func goroutine panic error, ", err)
	...
	for {
		...
		case pid := <-p.newPeers:
			...
			if p.whitelist.Size() > 0 && !p.whitelist.Contains(pid) {
				log.Warn("ignoring connection from not whitelisted peer: ", pid)
				continue
			}
...
func (p *PubSub) pushMsg(msg *Message) {
	...
	if p.whitelist.Size() > 0 {
		if p.signID != src {
			if !p.whitelist.Contains(src) {
				log.Warn("dropping message not from whitelisted peer: ", src)
				p.tracer.RejectMessage(msg, rejectNotWhitelistedPeer)
				return
			}
		}

		if p.signID != msg.GetFrom() {
			if !p.whitelist.Contains(msg.GetFrom()) {
				log.Warn("dropping message not from whitelisted source: ", src)
				p.tracer.RejectMessage(msg, rejectNotWhitelistedPeer)
				return
			}
		}
...
func (p *PubSub) publishMessage(msg *Message) {
	...
	if p.pubsubUid != msg.GetPubsubUid() {
		log.Warn("dropping message not belong to this pubsub: ", msg.PubsubUid)
		p.tracer.RejectMessage(msg, rejectNotBelongToThisPubsub)
		return
	}
...
// Can be removed once libp2ppubsub.Publish() and libp2ppubsub.Subscribe() are removed
...
// before the subscription is processed by the libp2ppubsub main loop and propagated to our peers.
//
...
//
// Deprecated: use libp2ppubsub.Join() and topic.Publish() instead
