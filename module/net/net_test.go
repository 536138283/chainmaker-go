...
func TestNet(t *testing.T) {
	...
	require.Nil(t, err)
	err = a.AddTrustRoot(chainId2, caBytes6666)
	require.Nil(t, err)
	err = a.AddTrustRoot(chainId2, caBytes7777)
	require.Nil(t, err)
	...
	require.Nil(t, err)
	err = b.AddTrustRoot(chainId2, caBytes6666)
	require.Nil(t, err)
	...
	// test A send msg to B and wait for response from B
	/*resHandlerB := func(id string, msg []byte) ([]byte, error) {
		fmt.Println("[B][chain1] recv a msg from peer[", id, "], msg：", string(msg))
		res := "world!"
		fmt.Println("[B][chain1] send the response msg to peer[", id, "], msg：", res)
		return []byte(res), nil
	}
	if err = b.ResponseMsg(chainId1, "TEST_SEND", resHandlerB); err != nil {
		t.Fatal(err)
	}
	fmt.Println("[B]B register response msg handler for chain1")
	result, err := a.SendWithResponseMsg(chainId1, toNodeB, "TEST_SEND", data, 10*time.Second)
	if err != nil {
		t.Fatal(err)
	}
	fmt.Println("[A]A recv a msg response from peer[", toNodeB, "], msg：", string(result))
	fmt.Println("==== test A send msg to B and wait for response from B pass ====")*/
	...
	sendMsg := NewNetMsg(data, netPb.NetMsg_TX, toNodeB)
	err = a.BroadcastWithChainId(chainId1, testTopic, sendMsg)
	require.Nil(t, err)
